...
try:
    ...
    st.error(f"❌ No se encontró el archivo en la ruta esperada: {RUTA_DATASET}")
    st.stop()
...
# =========================================================
# ENCABEZADO PRINCIPAL Y TITULAR (EL LINDO RECUADRO OSCURO)
# =========================================================
st.markdown("""
...
with kpi2:
    ...
    <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; text-align: center; border: 1px solid #e9ecef; box-shadow: 2px 2px 5px rgba(0,0,0,0.05);">
        ...
        <p style="color: #212529; font-size: 13px; margin-top: 5px;">Tasa de informalidad general</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    # Recuadro blanco de Diagnóstico Crítico (En Negro Nítido)
    st.markdown("""
    <div style="color: #000000; background-color: #ffffff; padding: 25px; border-radius: 8px; border: 1px solid #e9ecef; border-left: 5px solid #2ecc71; box-shadow: 2px 2px 5px rgba(0,0,0,0.02); margin-top: 15px;">
        ...
        <p style="color: #212529; font-size: 15px; line-height: 1.6;">La proporción interna de <strong>mujeres en situación de informalidad es alarmantemente superior</strong> en comparación con la distribución de los hombres.</p>
...
# ---------------------------------------------------------
# PESTAÑA 2: MOTIVACIONES (EL ÚNICO GRÁFICO CAMBIADO)
# ---------------------------------------------------------
with tab2:
    ...
    fig_mot = px.bar(
        ...
        text_auto=True # <-- Esto activa las etiquetas de los números
    )
    ...
    # ¡Aquí ocurre la magia! Mueve el número a la parte de afuera/derecha de cada barra horizontal
...
with tab3:
    ...
    fig_rama.update_traces(
        ...
        hole=0.3  # Estilo dona moderno que te gustó
    )
